...
if (![string]::IsNullOrEmpty($GeminiKey)) {
    gcloud run deploy $ServiceName `
      ...
      --update-env-vars GEMINI_API_KEY="$GeminiKey",USE_FIREBASE="true",FIREBASE_PROJECT_ID="$ProjectId"
} else {
    gcloud run deploy $ServiceName `
      --image gcr.io/$ProjectId/$ServiceName `
      --platform managed `
      --region $Region `
      --allow-unauthenticated `
      --update-env-vars USE_FIREBASE="true",FIREBASE_PROJECT_ID="$ProjectId"
}
